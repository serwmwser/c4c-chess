...
import {
  ...
  TIME_OPTIONS, STAKE_OPTIONS, CLASSIC_THEMES, LANGUAGES, t,
...
export default function Page() {
  ...
  useEffect(() => {
    if (profile.theme && isClient) { const t = (CLASSIC_THEMES as any)[profile.theme]
  ...
  if (!isClient) return <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)',color:'var(--text)'}}>{t(L,'loading')}</div>
  ...
  if (!isConnected) return <div style={{minHeight:'100vh',background:'var(--bg)',color:'var(--text)',padding:20,display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center'}}>
    ...
    <button onClick={()=>setShowModal(true)} disabled={isPending} style={{padding:'16px 48px',background:isPending?'#6b7280':'var(--accent)',borderRadius:12,fontSize:18}}>{isPending?'⏳...':t(L,'connect')}</button>
    {showModal && <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShowModal(false)}>
      <div style={{background:'var(--card)',padding:24,borderRadius:16,maxWidth:360,width:'100%'}} onClick={(e:any)=>e.stopPropagation()}>
        <h3 style={{textAlign:'center',marginBottom:20}}>{t(L,'chooseWallet')}</h3>
        ...
        <button onClick={()=>setShowModal(false)} style={{width:'100%',padding:12,marginTop:16,background:'var(--border)',borderRadius:8}}>{t(L,'close')}</button>
      </div>
    </div>}
  </div>
  ...
  return <div style={{minHeight:'100vh',background:'var(--bg)',color:'var(--text)',padding:20}}>
    <div style={{maxWidth:700,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        ...
        <button onClick={()=>disconnect()} style={{background:'var(--error)',padding:'8px 16px',borderRadius:6,color:'#fff'}}>{t(L,'logout')}</button>
      </div>
      <div style={{display:'flex',gap:8,marginBottom:16}}>
        {['profile','lobby','friends'].map((t:any)=><button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:10,background:tab===t?'var(--accent)':'var(--card)',borderRadius:8,color:tab===t?'#000':'var(--text)'}}>{t==='profile'?t(L,'profile'):t==='lobby'?t(L,'lobby'):t(L,'friends')}</button>)}
      </div>
      {tab==='profile' && <div style={{background:'var(--card)',padding:20,borderRadius:16}}>
        <div style={{display:'flex',gap:16,alignItems:'flex-start',marginBottom:16}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:13,opacity:.7}}>{t(L,'balance')}: <span style={{fontSize:18,fontWeight:'bold',color:'var(--accent)'}}>{formatC4C(balance?.value)} C4C</span></p>
          </div>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          <label style={{fontSize:12,opacity:.7}}>{t(L,'desc')}</label>
          ...
          <label style={{fontSize:12,opacity:.7}}>{t(L,'theme')}</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>{Object.entries(CLASSIC_THEMES).map(([id,t]:any)=>(<button key={id} onClick={()=>updateProfile({theme:id})} style={{flex:1,padding:6,borderRadius:6,background:t.bg,color:t.text,border:profile.theme===id?'3px solid var(--accent)':'1px solid var(--border)'}}>{t.name}</button>))}</div>
          <label style={{fontSize:12,opacity:.7}}>🌐 Язык</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>{Object.entries(LANGUAGES).map(([id,l]:any)=>(<button key={id} onClick={()=>updateProfile({lang:id})} style={{flex:1,padding:6,borderRadius:6,background:'var(--card)',border:profile.lang===id?'2px solid var(--accent)':'1px solid var(--border)'}}>{l.flag} {l.name}</button>))}</div>
      ...
      {tab==='lobby' && <div style={{background:'var(--card)',padding:20,borderRadius:16}}>
        <h3 style={{marginBottom:12}}>{t(L,'lobby')}</h3>
        {games.length===0 ? <p style={{opacity:.7}}>{t(L,'noGames')}</p> : games.map((g:any)=>(<div key={g.id} style={{padding:10,background:'var(--bg)',borderRadius:8,marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><p style={{fontWeight:600}}>{t(L,'stake')}: {formatC4C(g.stake)}</p><p style={{fontSize:12,opacity:.6}}>{t(L,'time')}: {formatTime(g.timeControl)}</p></div><button onClick={()=>{reset();setTimeCtrl(g.timeControl);setStake(g.stake);setTab('profile')}} style={{padding:'6px 14px',background:'#3b82f6',color:'#fff',borderRadius:6}}>{t(L,'play')}</button></div>))}
      </div>}
      {tab==='friends' && <div style={{background:'var(--card)',padding:20,borderRadius:16}}>
        <h3 style={{marginBottom:12}}>{t(L,'friends')}</h3>
        <div style={{display:'flex',gap:8,marginBottom:12}}><input value={newFriendAddr} onChange={(e:any)=>setNewFriendAddr(e.target.value)} placeholder="Адрес"/><button onClick={()=>{if(newFriendAddr.length>3){addFriend({address:newFriendAddr,name:`Friend ${newFriendAddr.slice(2,6)}`,status:'online',addedAt:Date.now()});setFriends(getFriends());setNewFriendAddr('')}}} style={{background:'#3b82f6',color:'#fff',padding:'0 16px',borderRadius:6}}>{t(L,'addFriend')}</button></div>
        {friends.length===0 ? <p style={{opacity:.7}}>{t(L,'noFriends')}</p> : friends.map((f:any)=>(<div key={f.address} style={{padding:10,background:'var(--bg)',borderRadius:8,marginBottom:6,display:'flex',justifyContent:'space-between',alignItems:'center'}}><span>{f.name}</span><button onClick={()=>alert(`${t(L,'invite')} ${f.name}`)} style={{padding:'4px 10px',background:'var(--success)',color:'#fff',borderRadius:4,fontSize:12}}>{t(L,'invite')}</button></div>))}
      </div>}
      <div style={{marginTop:16,background:'var(--card)',padding:16,borderRadius:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(8,1fr)',gap:0,maxWidth:360,margin:'0 auto',borderRadius:8,overflow:'hidden',boxShadow:'0 4px 12px rgba(0,0,0,.4)'}}>
          {['8','7','6','5','4','3','2','1'].map((r,ri)=>['a','b','c','d','e','f','g','h'].map((f,fi)=>{
            ...
            const boardT = (SAFE_BOARD_THEMES as any)[profile.boardTheme] || SAFE_BOARD_THEMES.blue;
        ...
        <div style={{textAlign:'center',marginTop:12}}>{over && <p style={{color:'var(--error)',fontWeight:'bold',fontSize:16}}>{over}</p>}<button onClick={reset} style={{padding:'8px 20px',background:'#3b82f6',color:'#fff',borderRadius:6}}>{t(L,'newGame')}</button></div>
      </div>
    </div>
  </div>
}
